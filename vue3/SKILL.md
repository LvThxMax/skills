...
4. **DOM 扁平** — 模板层级尽量浅，避免无意义 wrapper；布局用 flex/grid，不为样式多包 div
5. **不用 BEM** — CSS 类名扁平、语义化（如 `user-card`、`is-active`），禁止 `block__element--modifier`

## 技术约定

- `<script setup lang="ts">` + Composition API
- Pinia 管状态，Vue Router 4/5 管路由（v5 对 v4 无破坏性变更，可直接升级），Vite 构建
